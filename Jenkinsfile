...
pipeline {
    ...
    environment {
        NEXUS_URL = 'http://localhost:8081/service/rest/v1/components'
        REPOSITORY = 'dist'
        GROUP = 'com.middlewaretalents'
        ARTIFACT_PREFIX = 'middlewaretalents'
        BUILD_DIR = 'dist'
        VERSION_INCREMENT = 1
        ARTIFACT_FILE = ''
        LATEST_VERSION = ''
    ...
    stages {
        stage('Get Latest Artifact Version from Nexus') {
            steps {
                script {
                    ...
                    def nexusResponse = sh(script: """curl -u admin:vamsi@123 -s "$NEXUS_URL?repository=$REPOSITORY&group=$GROUP" """, returnStdout: true).trim()
                    def jsonResponse = readJSON text: nexusResponse

                    // Find the latest version from Nexus response
                    def latestArtifact = jsonResponse.items.sort { a, b -> b.name <=> a.name }[0]
                    def version = latestArtifact.name.split('-')[1].replaceAll('[^0-9.]', '') // Extract version number

                    echo "Latest version from Nexus: $version"
                    LATEST_VERSION = version
                    VERSION_INCREMENT = version.toDouble() + 0.1 // Increment version by 0.1
                }
            }
        }
        ...
        stage('Create .zip Archive') {
            steps {
                script {
                    echo "Creating .zip Archive for the build..."
                    sh "zip -r ${ARTIFACT_PREFIX}-${VERSION_INCREMENT}.zip ${BUILD_DIR}"
                    ARTIFACT_FILE = "${ARTIFACT_PREFIX}-${VERSION_INCREMENT}.zip"
                }
            }
        }
        ...
        stage('Upload to Nexus') {
            steps {
                script {
                    ...
                    sh """
                        curl -u admin:vamsi@123 --upload-file ${ARTIFACT_FILE} \\
                            http://localhost:8081/repository/$REPOSITORY/${ARTIFACT_FILE}
                    """
                }
            }
        }
