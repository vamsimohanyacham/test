...
pipeline {
    ...
    environment {
        ...
        AZURE_APP_NAME = 'eshwar-test'  // Azure Web App Name (Change this)
        ZIP_FILE = "${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip"  // Zip file for Azure Web App deployment
        DEPLOY_PATH = 'D:\\deployments' // Define a deployment path for storing the zip file
        DIST_PATH = "${WORKSPACE}\\dist" // Path to the dist folder generated by the build
    }
    ...
    stages {
        ...
        stage('Build Vite App') {
            steps {
                ...
                bat 'dir'  // List workspace contents to ensure dist folder is created
            }
        }

        stage('Check Build Directory') {
            steps {
                bat 'dir dist'  // List the dist directory contents to verify the build output
            }
        }
        ...
        stage('Create .zip Archive') {
            steps {
                script {
                    // Check if dist folder exists and print its contents
                    bat 'dir dist'
                    
                    // Create a .zip file from the dist folder
                    bat """
                        powershell Compress-Archive -Path dist\\* -DestinationPath ${DEPLOY_PATH}\\${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip
                    """
                    echo "Created ${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip at ${DEPLOY_PATH}"
                }
            }
        }
        ...
        stage('Upload to Nexus') {
            steps {
                script {
                    ...
                    bat """
                        echo Uploading ${artifactFile} to Nexus...
                        curl -u ${NEXUS_USER}:${NEXUS_PASSWORD} -X PUT -F "file=@${DEPLOY_PATH}\\${artifactFile}" ${nexusRepositoryUrl}
                    """
                }
            }
        }
        ...
        stage('Move dist to Nginx Directory') {
            steps {
                script {
                    // Verify that dist exists before moving it
                    bat 'dir dist'  // List the contents of the dist folder
                    // Move dist folder to Nginx directory
                    bat "xcopy /E /I /H /Y dist C:\\Users\\MTL1020\\Downloads\\nginx-1.26.2\\nginx-1.26.2\\html\\"
                    echo "Moved dist folder to Nginx directory"
                }
            }
        }
        ...
        stage('Deploy to Azure') {
            steps {
                script {
                    ...
                    // Deploy to Azure if dist folder exists
                    bat """
                        if exist ${distPath} (
                            az webapp deploy --resource-group ${AZURE_RESOURCE_GROUP} --name ${AZURE_APP_NAME} --src-path ${distPath} --type static
                        ) else (
                            echo dist folder does not exist, skipping Azure deployment
                        )
                    """
                }
            }
