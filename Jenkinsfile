...
pipeline {
    ...
    environment {
        ...
        AZURE_APP_NAME = 'eshwar-test'  // Azure Web App Name (Change this)
        ZIP_FILE = "${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip"  // Zip file for Azure Web App deployment
        DEPLOY_DIR = 'D:\\deployments'  // New directory for deployment files
    }
    ...
    stages {
        ...
        stage('Create .zip Archive') {
            steps {
                script {
                    bat 'dir dist'
                    bat "powershell Compress-Archive -Path dist\\* -DestinationPath ${DEPLOY_DIR}\\${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip"
                    echo "Created ${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip from dist folder"
                    bat 'dir'
                }
            }
        ...
        stage('Upload to Nexus') {
            steps {
                script {
                    ...
                    bat """
                        echo Uploading ${artifactFile} to Nexus...
                        curl -u ${NEXUS_USER}:${NEXUS_PASSWORD} -X PUT -F "file=@${DEPLOY_DIR}\\${artifactFile}" ${nexusRepositoryUrl}
                    """
                }
            }
        }
        ...
        stage('Extract Artifact from Nexus') {
            steps {
                script {
                    echo "Extracting ${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip..."
                    bat "powershell Expand-Archive -Path ${DEPLOY_DIR}\\${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip -DestinationPath ${DEPLOY_DIR}"
                    echo "Artifact extracted"
                }
            }
        }
        ...
        stage('Move dist to Nginx Directory') {
            steps {
                script {
                    // Move to the new path (D:\deployments)
                    bat "xcopy /E /I /H /Y ${DEPLOY_DIR}\\dist ${NGINX_PATH}\\"
                    echo "Moved dist folder to Nginx directory"
                }
            }
        }
        ...
        stage('Deploy to Azure') {
            steps {
                script {
                    def artifactFile = "${ARTIFACT_NAME}-${ARTIFACT_VERSION}.zip"
                    def artifactPath = "${DEPLOY_DIR}\\${artifactFile}"  // Path to the new directory
                    def distPath = "${DEPLOY_DIR}\\dist"  // This is the directory containing your built files

                    echo "Extracting ${artifactFile} from ${artifactPath}..."
                    bat "powershell Expand-Archive -Path ${artifactPath} -DestinationPath ${DEPLOY_DIR}"
